...
public class HoverBike : MonoBehaviour
{
    ...
    private float velocityYSmooth = 0f;
    ...
    private Vector3 velocitySmooth = Vector3.zero;                                              // Stocke la vélocité lissée pour les trois axes
    public float rotSmoothTime = 0.3f;                                                             // Temps de lissage pour ajuster la rotation
    ...
    void FixedUpdate()
    {
        ...
        //change pour les test de la moto uniquement 
        if (/*playerMount*/ true)
        {
            //AdjustPlayerPos();
    ...
    private void AdjustRotation()
    {
        ...
        else if (hitCount <= 1)
        {
            ...
            targetRotation = new Vector3(0f, transform.rotation.eulerAngles.y, zRotation);
    ...
    float AdjustVelocityY()
    {
        if (hover)
        {
            ...
            float targetVolocityY = distanceAjusteeAuSol * vitesseAjustement;

            return Mathf.SmoothDamp(rb.velocity.y, targetVolocityY, ref velocityYSmooth, smoothTime);
        }
        else
        {
